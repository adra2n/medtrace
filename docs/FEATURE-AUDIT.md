...
| 序号 | 问题 | 优先级 | 状态 | 类型 |
|------|------|--------|------|------|
| A1 | VIP 免费限制未执行 | 🔴 高 | ✅ 已完成 | 功能缺失 |
| C1 | 用药提醒支持具体时间点 | 🔴 高 | ✅ 已完成 | 功能增强 |
| A2 | 疗程进度 UI 入口 | 🟡 中 | ✅ 已完成 | 功能补全 |
| B1 | NavGraph 死代码清理 | 🟡 中 | ✅ 已完成 | 架构清理 |
| B3 | RemindersViewModel 绕过 Repository | 🟡 中 | ✅ 已完成 | 架构修复 |
| B4 | MedicalRecordViewModel 搜索参数忽略 | 🟡 中 | ✅ 已完成 | 架构修复 |
| C2 | 提醒页按成员筛选 | 🟡 中 | ✅ 已完成 | 功能增强 |
| C4 | 病历附件图片 | 🟡 中 | ✅ 已完成 | 功能增强 |
| A3 | 通知声音/震动设置 | 🟢 低 | ✅ 已完成 | 功能补全 |
| B2 | SettingsViewModel 空壳 | 🟢 低 | ✅ 已完成 | 架构修复 |
| C3 | 趋势页自定义时间范围 | 🟢 低 | ✅ 已完成 | 功能增强 |
| C5 | 病历排序选项 | 🟢 低 | ✅ 已完成 | 功能增强 |
| C6 | Excel 导出 | 🟢 低 | ✅ 已完成 | 功能增强 |
...
1. **核心功能完成度 ~95%**，作为记录工具功能完整
2. **VIP 体系已激活**：免费限制（2 成员/3 次 AI）已执行，注册码流程可用
3. **用药提醒升级**：支持精确时间点（8:00/12:00/20:00），不再只有 9:00 汇总
4. **架构清理完成**：死代码删除、ViewModel-Repository 对齐、搜索逻辑下沉
5. **工具类增强**：疗程进度、成员筛选、时间范围、排序选项、附件图片、Excel 导出
