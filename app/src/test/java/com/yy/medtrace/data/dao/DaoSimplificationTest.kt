...
class DaoSimplificationTest {
    @Test
    fun `FamilyMemberDao should have all required methods`() {
        // 验证DAO方法包含所有必需的方法
        val interfaceMethods = FamilyMemberDao::class.java.declaredMethods.map { it.name }
    ...
    @Test
    fun `MedicalRecordDao should have all required methods`() {
        // 验证DAO方法包含所有必需的方法
        val interfaceMethods = MedicalRecordDao::class.java.declaredMethods.map { it.name }
